...
layout_grid.SparkR <- function(data, rows = NULL, cols = NULL, margins = NULL, drop = TRUE, as.table = TRUE) {
  ...
  if(!rows_is_null && !cols_is_null) {
    ...
    rows_value <- SparkR::mutate(rows_value, row1 = rows_value$"_1", id = cast(rows_value$"_2", "integer"))

    cols_value <- bindIDs(cols_value)
    cols_value <- SparkR::mutate(cols_value, col1 = cols_value$"_1", PANEL = cast(cols_value$"_2", "integer"))
    ...
    panels <- withColumn(panels, "COL", lit(1))
  } else if(!cols_is_null) {
    panels <- withColumn(cols, "PANEL", cols$COL)
    panels <- withColumn(panels, "ROW", lit(1))
  }
