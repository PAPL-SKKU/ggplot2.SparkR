...
ggplot.default <- function(data = NULL, mapping = aes(), ..., environment = parent.frame()) {
  print("MK_run ggplot.default() in plot.r")

  #' MK_Test
  if(is.data.frame(data)) {
    ggplot.data.frame(fortify(data, ...), mapping, environment = environment)
  } else {
    ggplot.DataFrame(data, mapping, environment = environment)
  } 
}
...
ggplot.data.frame <- function(data, mapping=aes(), ..., environment = parent.frame()) {
  print("MK_run ggplot.data.frame() in plot.r")
...
#' MK_Create a new ggplot plot from a Spark DataFrame
ggplot.DataFrame <- function(data, mapping=aes(), ..., environment = parent.frame()) {
  print("MK_run ggplot.DataFrame() in plot.r-1")
  if (!missing(mapping) && !inherits(mapping, "uneval")) stop("Mapping should be created with aes or aes_string")
  print("MK_run ggplot.DataFrame() in plot.r-2")
  p <- structure(list(
    data = data,
    layers = list(),
    scales = Scales$new(),
...
plot_clone <- function(plot) {
  print("MK_run plot_clone() in plot.r")
  p <- plot
